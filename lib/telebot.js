import standardUpdates from './updates.js';
import standardMethods from './methods.js';
...
class TeleBot {
    ...
    constructor(cfg) {
        ...
        // this.buildInPlugins.map(buildInPluginName => this.plug(import(`${this.buildInPluginsFolder}${buildInPluginName}`)));

        // Load user plugins
        // this.usePlugins.map(userPluginName => this.plug(import(`${this.pluginFolder}${userPluginName}`)));
    ...
    async request(url, form, data) {
        ...
        if (data) {
            ...
            for (let item in data) {
                ...
                options.body.append(item, data[item]);
            }
        }
